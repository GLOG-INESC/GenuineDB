...
std::pair<shared_ptr<SpeculativeMemOnlyStorage>, shared_ptr<MetadataInitializer>> MakeSpecStorage(const ConfigurationPtr& config,
                                                                                   const string& data_dir) {
  auto storage = std::make_shared<SpeculativeMemOnlyStorage>();
  shared_ptr<MetadataInitializer> metadata_initializer;
  switch (config->proto_config().partitioning_case()) {
    case internal::Configuration::kSimplePartitioning:
      metadata_initializer = make_shared<SimpleMetadataInitializer>(config->num_regions(), config->num_partitions());
...
void GenerateSimpleData(shared_ptr<Storage> storage, const shared_ptr<MetadataInitializer>& metadata_initializer,
  ...
  auto region = config->local_region();
...
#elif PARTIAL_REP
for (uint64_t key = from_key; key < to_key; key += num_partitions*num_regions) {
#endif
      Record record(value);
...
#elif PARTIAL_REP
    uint64_t partition_of_range_start = range_start % (num_partitions*num_regions);
    uint64_t distance_to_next_in_partition_key =
        (partition+region*num_partitions - partition_of_range_start + num_partitions*num_regions) % (num_partitions*num_regions);
    uint64_t from_key = range_start + distance_to_next_in_partition_key;
#endif
...
#elif PARTIAL_REP
  tpcc::LoadTables(storage_adapter, tpcc_partitioning.warehouses(), config->num_regions(), config->num_partitions(),
                   config->local_partition(), config->local_region(), kDataGenThreads);
#endif
}
