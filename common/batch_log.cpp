...
void BatchLog::AddSlot(SlotId slot_id, BatchId batch_id, int replication_factor, int64_t ordering_arrival_time) {
  auto time = std::chrono::system_clock::now().time_since_epoch().count();
  slots_.Insert(slot_id, std::make_tuple(batch_id, time, ordering_arrival_time));
...
std::pair<SlotId, BatchPtr> BatchLog::NextBatch() {
  ...
  auto [next_batch_id, time, ordering_arrival_time] = ready_batches_.front().second;
  ready_batches_.pop();

  auto it = batches_.find(next_batch_id);

  //RECORD_WITH_TIME(it->second.get(), TransactionEvent::ENTER_LOG_MANAGER_ORDER, time);
  // Calculate the time spent on ordering layer
  it->second->set_ordering_sent(ordering_arrival_time);
...
void BatchLog::UpdateReadyBatches() {
  while (slots_.HasNext()) {
    auto next_batch_id = std::get<0>(slots_.Peek());
