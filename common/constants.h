...
// Worker channels are in [kWorkerChannel, kZipOrderChannel)
...
const Channel kZipOrderChannel = 70;
const Channel kZipTxnGenerator = 71;

const Channel kMultiHomeOrdererChannel = 111;

const Channel kGLOGChannel = 130;

const Channel kGLOGStubChannel = 140;

const Channel kMaxChannel = 200;
...
const uint32_t kMaxNumMachines = 210;
...
constexpr Channel kMaxNumWorkers = kZipOrderChannel - kWorkerChannel;
...
// Maximum number of regions relevant for bitmap
static constexpr unsigned int JANUS_MAX_NUM_REGIONS = 8;
static constexpr unsigned long JANUS_MAX_NUM_PARTITIONS = 8;
...
/** number of maximum Regions */
static constexpr unsigned int MAX_NUM_REGIONS = (1<<8) -1;

static constexpr unsigned long MAX_NUM_PARTITIONS = 32;

/** number of maximum clients allowed per region */
static constexpr unsigned int MAX_NUM_GEN_PER_REGION = 64;

/** number of maximum ziplog stubs per generator. Must at least be two (one for each stub type) */

static constexpr unsigned int MAX_NUM_ZIP_STUBS = 2;
