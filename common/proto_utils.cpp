...
Transaction* GeneratePartitionedTxn(const SharderPtr& sharder, Transaction* txn, uint32_t partition, bool in_place) {
  ...
  // add client id to new transaction
  new_txn->mutable_internal()->set_client_id(txn->internal().client_id());

  vector<bool> involved_regions(MAX_NUM_REGIONS, false);
...
Transaction* GeneratePartitionedTxn(const SharderPtr& sharder, Transaction* txn, RegionId region, uint32_t partition, bool in_place) {
  ...
  auto* keys = new_txn->mutable_keys();
  int i = 0;

  while (i < keys->size()) {
    auto& entry = (*keys)[i];
    bool keep = (entry.value_entry().metadata().master() == region) &&
                (sharder->compute_partition(entry.key()) == partition);

    if (keep) {
      i++;  // keep this element, move to next
    } else {
      // Swap with the last element and remove last
      int last_index = keys->size() - 1;
      if (i != last_index) {
        keys->SwapElements(i, last_index);
      }
      keys->RemoveLast();
      // Do NOT increment i, because the swapped element at i needs to be checked
    }
  }
  ...
  for (auto it = new_txn->mutable_keys()->begin(); it != new_txn->mutable_keys()->end();) {
    ...
    const bool keep = (
        (it->value_entry().metadata().master() == region) &&
        (sharder->compute_partition(key) == partition)
        );
...
void PopulateInvolvedMetadata(std::shared_ptr<slog::MetadataInitializer>& metadata_initializer, Transaction& txn){
  for (auto& kv : *txn.mutable_keys()) {
    const auto& key = kv.key();
    auto value = kv.mutable_value_entry();

    auto metadata = metadata_initializer->Compute(kv.key());
...
void PopulateInvolvedPartitions(const SharderPtr& sharder, Transaction& txn) {
  ...
  std::unordered_set<MachineId> involved_partitions{};
  std::unordered_set<MachineId> active_partitions{};

  for (const auto& kv : txn.keys()) {

    PartitionId partition = sharder->compute_partition(kv.key());

    MachineId machine_id = MakeMachineId(kv.value_entry().metadata().master(), 0, partition);

    involved_partitions.insert(machine_id);

    if (kv.value_entry().type() == KeyType::WRITE) {
      active_partitions.insert(machine_id);
    }
  }

  auto* internal = txn.mutable_internal();
  internal->mutable_involved_partitions()->Reserve((int)involved_partitions.size());
  internal->mutable_active_partitions()->Reserve((int)active_partitions.size());


  for (const auto & partition : involved_partitions){
    txn.mutable_internal()->add_involved_partitions(partition);
  }

  for (const auto & partition : active_partitions){
    txn.mutable_internal()->add_active_partitions(partition);
  }
}

void MergeTransaction(Transaction& txn, const Transaction& other, bool merge_events) {
  ...
  //txn.mutable_internal()->mutable_holder_txns()->MergeFrom(other.internal().holder_txns());
  //txn.mutable_internal()->mutable_queue_txns()->MergeFrom(other.internal().queue_txns());
...
vector<Transaction*> Unbatch(Batch* batch) {
...
vector<Transaction*> Unbatch(std::shared_ptr<Batch> batch) {
  auto transactions = batch->mutable_transactions();
