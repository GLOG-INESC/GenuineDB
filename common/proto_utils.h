...
/**
 * Takes into account the local region to filter keys
 */
...
/**
 * Populate the involved metadata of each key
 */
 ...
 * @param merge_events Flag determining whether to merge the events as well
 */
void MergeTransaction(Transaction& txn, const Transaction& other, bool merge_events = true);
 ...
 */
std::vector<Transaction*> Unbatch(Batch* batch);
std::vector<Transaction*> Unbatch(std::shared_ptr<Batch> batch);
