...
class MultiHomeOrderer : public NetworkedModule {
 public:
  MultiHomeOrderer(const std::shared_ptr<zmq::context_t>& context, const ConfigurationPtr& config, const MetricsRepositoryManagerPtr& metrics_manager,
 ...
 private:
  ...
  std::vector<std::unique_ptr<Batch>> batch_per_reg_;
