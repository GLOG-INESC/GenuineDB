...
bool RecordFinishedTxn(TxnGenerator::TxnInfo& info, ClientId client_id, Transaction* txn) {
  if (info.finished) {
    LOG(ERROR) << "[TXN_GEN_" << client_id << "] Received response for finished txn";
    return false;
  }
...
int TxnGenerator::genId() const {return id_; }
...
void SynchronousTxnGenerator::SetUp() {
  ...
  if (!dry_run_) {
    ...
    for (int i = 0; i < num_clients_; i++) {
      // Add a delay across 1 second
      std::uniform_int_distribution<std::mt19937::result_type> dist100(1,10000);
      // Delay in microseconds
      auto delay = dist100(rng)*1000;

      poller_.AddTimedCallback(microseconds(delay), [this] { SendNextTxn(); });
    }
  }
...
void SynchronousTxnGenerator::SendTxnWithDelay(TxnId id) {
  std::uniform_int_distribution<std::mt19937::result_type> dist100(1,config_->txn_issuing_delay());
  // Delay in microseconds
  auto delay = dist100(rng)*1000;

  poller_.AddTimedCallback(microseconds(delay), [this, id] { SendTxn(id); });
}
...
bool SynchronousTxnGenerator::Loop() {
  ...
  bool stuck_reached = elapsed_time()  >= duration_*2;
  ...
  if (duration_reached && (aborted_txns() + committed_txns() == txns_.size() || stuck_reached)) {
    StopTimer();
    return true;
  }
  return false;
...
bool ConstantRateTxnGenerator::Loop() {
  ...
  if (duration_ > 0ms) {
    bool duration_reached = elapsed_time() >= duration_;
    stop = duration_reached && (dry_run_ || recv_txns == sent_txns() || elapsed_time() >= (duration_+10s));
  } else {
    stop = sent_txns() >= generated_txns_.size() && (dry_run_ || recv_txns >= txns_.size() || elapsed_time() >= (duration_+10s));
  }
  if (stop) {
    StopTimer();
    return true;
