...
class NetworkedModule : public Module {
 ...
 protected:
  ...
  // Returns true if the application wishes to exit
  virtual bool OnSignalEnd() { return false; }
  ...
  virtual void Send(const internal::Envelope& env, MachineId to_machine_id, Channel to_channel);
  virtual void Send(EnvelopePtr&& env, MachineId to_machine_id, Channel to_channel);
  virtual void Send(EnvelopePtr&& env, Channel to_channel);
  virtual void Send(const internal::Envelope& env, const std::vector<MachineId>& to_machine_ids, Channel to_channel);
  virtual void Send(EnvelopePtr&& env, const std::vector<MachineId>& to_machine_ids, Channel to_channel);
