...
MultiHomeOrderer::MultiHomeOrderer(const std::shared_ptr<zmq::context_t>& context, const ConfigurationPtr& config, const MetricsRepositoryManagerPtr& metrics_manager,
                                   std::chrono::milliseconds poll_timeout)
    : NetworkedModule(context, config,  config->mh_order_port(),kMultiHomeOrdererChannel, metrics_manager, poll_timeout, true /* is_long_sender */),
      batch_id_counter_(0) {
...
void MultiHomeOrderer::ProcessForwardBatchData(EnvelopePtr&& env) {
  ...
  for (auto txn_it = 0; txn_it < batch->transactions_size(); txn_it++) {
    RECORD(batch->mutable_transactions(txn_it)->mutable_internal(), TransactionEvent::ENTER_MULTI_HOME_ORDERER_IN_BATCH);
  }
...
void MultiHomeOrderer::AdvanceLog() {
  ...
  if (multi_home_batch_log_.HasNextBatch()){
    auto env = NewEnvelope();
    auto slog_batch = env->mutable_request()->mutable_slog_batch_forward();
    do {
      auto batch_and_slot = multi_home_batch_log_.NextBatch();
      auto txns = batch_and_slot.second->mutable_transactions();
      ...
      if (txns->size() > 0){
        slog_batch->mutable_batches()->AddAllocated(batch_and_slot.second.release());
      }
