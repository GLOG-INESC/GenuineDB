...
class Batcher : public NetworkedModule {
 ...
 private:
  ...
  using PartitionedBatch = std::vector<std::unique_ptr<Batch>>;

  void ProcessReadyFutureTxns();
  void StartOver();
  void NewBatch();
  void BatchTxn(Transaction* txn);
  void MergeBatch(EnvelopePtr&& env);
  void MergeSLOGBatch(EnvelopePtr&& env);

  BatchId batch_id() const;
  void SendBatches();
  EnvelopePtr NewBatchForwardingMessage(int generator_position, int64_t ordering_sent_timestamp);
  EnvelopePtr NewBatchForwardingMessage(std::vector<Batch*>&& batch, int generator_position,
                                        int64_t ordering_sent_timestamp);
  EnvelopePtr NewBatchForwardingMessage(std::vector<EnvelopePtr>&& envs, int generator_position,
                                        int64_t ordering_sent_timestamp);
