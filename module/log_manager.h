...
class LocalLog {
 public:
  ...
  void AddSlot(SlotId slot_id, uint32_t queue_id, MachineId leader, int64_t order_arrived_time);

  bool HasNextBatch() const;
  std::pair<SlotId, std::tuple<BatchId, MachineId, long>> NextBatch();
 ...
 private:
  ...
  AsyncLog<std::tuple<uint32_t, MachineId, long>> slots_;
  // Batches from a partition form a queue
  std::unordered_map<uint32_t, AsyncLog<BatchId>> batch_queues_;
  // Chosen batches
  std::queue<std::pair<SlotId, std::tuple<BatchId, MachineId, long>>> ready_batches_;
};
...
class LogManager : public NetworkedModule {
 ...
 private:
  ...
  // Log module identifier for debugging purposes
  std::string log_manager_debug_;
