...
Scheduler::Scheduler(const shared_ptr<Broker>& broker, const shared_ptr<Storage>& storage,
  ...
  std::stringstream scheduler_debug_tag;

  scheduler_debug_tag << "[SCHEDULER_" << (int)config()->local_region() << "_" << (int)config()->local_replica() << "_" << config()->local_partition() << "] ";
  scheduler_debug_tag_ = scheduler_debug_tag.str();

  for (int i = 0; i < config()->num_workers(); i++) {
    workers_.push_back(MakeRunnerFor<Worker>(i, broker, storage, metrics_manager, poll_timeout, scheduler_debug_tag_));
  }
...
#ifdef LOCK_MANAGER_DDR_PESSIMISTIC

        auto it = active_txns_.find(ready_txn.first);
        DCHECK(it != active_txns_.end());

        VLOG(3) << "Txn " << TXN_ID_STR(ready_txn.first) << " became ready after resolving a deadlock";

        // The ready txns list contains txns that got into a resolved deadlock
        Dispatch(ready_txn.first, ready_txn.second /* deadlocked */, false /* is_fast */);
#else
...
void Scheduler::UnblockTransactions(TxnId txn_id) {
  uint64_t got_request = std::chrono::high_resolution_clock::now().time_since_epoch().count();
  ...
  VLOG(3) << scheduler_debug_tag_ << "Released locks of txn " << TXN_ID_STR(txn_id);
  ...
  uint64_t unlocked_all = std::chrono::high_resolution_clock::now().time_since_epoch().count();
  if (per_thread_metrics_repo != nullptr){
    per_thread_metrics_repo->RecordUnlockingTxns(txn_id, got_request, unlocked_all);
  }
...
void Scheduler::ProcessTransaction(EnvelopePtr&& env) {
  ...
  // If the txn is from another home, we just add it to the graph without locks

#if defined(PARTIAL_EXEC) && defined(FULL_REP)
  // When partially executing but fully replicating, one must skip transaction execution of replicated single homes
  bool involved = false;
  for( auto involved_partitions : txn->internal().involved_partitions()) {
    involved |= involved_partitions == config()->local_machine_id();
  }

  // If not involved, release the locks and delete the txn
  if (!involved) {
    VLOG(3) << scheduler_debug_tag_ << "Transaction (" << TXN_ID_STR(txn_id) << ") is not involved in this partition";
  ...
  if (ins.second) {
    ...
    VLOG(3) << scheduler_debug_tag_ << "Accepted " << ENUM_NAME(txn->internal().type(), TransactionType) << " transaction ("
            << TXN_ID_STR(txn_id) << ", " << txn->internal().home() << ")";
  } else {
    if (!holder.AddLockOnlyTxn(txn)) {
      LOG(ERROR) << scheduler_debug_tag_ << "Already received txn: (" << TXN_ID_STR(txn_id) << ", " << txn->internal().home() << ")";
      return;
    }
    ...
    VLOG(3) << scheduler_debug_tag_ << "Added " << ENUM_NAME(txn->internal().type(), TransactionType) << " transaction (" << TXN_ID_STR(txn_id)
            << ", " << txn->internal().home() << ")";
  }
...
void Scheduler::SendToRemasterManager(Transaction& txn) {
  switch (remaster_manager_.VerifyMaster(txn)) {
    ...
    case VerifyMasterResult::WAITING: {
      VLOG(3) << scheduler_debug_tag_ << "Txn waiting on remaster: " << TXN_ID_STR(txn.internal().id());
      // Do nothing
      break;
    }
    default:
      LOG(ERROR) << scheduler_debug_tag_ << "Unknown VerifyMaster type";
      break;
  }
}
...
void Scheduler::SendToLockManager(Transaction& txn) {
  ...
  VLOG(3) << scheduler_debug_tag_ << "Trying to acquires locks of txn " << TXN_ID_STR(txn_id);
  ...
  switch (lock_manager_.AcquireLocks(txn)) {
    ...
    case AcquireLocksResult::WAITING:
      VLOG(3) << scheduler_debug_tag_ << "Txn " << TXN_ID_STR(txn_id) << " cannot be dispatched yet";
      break;
    default:
      LOG(ERROR) << scheduler_debug_tag_ << "Unknown lock result type";
      break;
  }
}
...
void Scheduler::Dispatch(TxnId txn_id, bool deadlocked, bool is_fast) {
  auto it = active_txns_.find(txn_id);
  CHECK(it != active_txns_.end()) << scheduler_debug_tag_ << "Txn " << txn_id << " does not exist for dispatching";
  auto& txn_holder = it->second;

  CHECK(txn_holder.dispatchable()) << scheduler_debug_tag_ << "Can no longer dispatch txn " << TXN_ID_STR(txn_holder.txn_id())
  ...
  VLOG(3) << scheduler_debug_tag_ << "Dispatched txn " << TXN_ID_STR(txn_id) << " (deadlocked = " << deadlocked << ")";
  // Transaction must be prepared according to the used replication
...
void Scheduler::TriggerPreDispatchAbort(TxnId txn_id, const std::string& abort_reason) {
  ...
  VLOG(3) << scheduler_debug_tag_ << "Triggering pre-dispatch abort of txn " << TXN_ID_STR(txn_id);
