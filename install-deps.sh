...
$SUDO apt-get update
$SUDO apt-get -y install autoconf automake libtool curl unzip libreadline-dev pkg-config wget iputils-ping cmake build-essential || true
...
if need_install 'gprofng'; then
  gprofng_ver=2.43
  mkdir -p $DOWNLOAD_DIR
    cd $DOWNLOAD_DIR
    echo "Downloading gprofng"
    wget https://ftp.gnu.org/gnu/binutils/binutils-${gprofng_ver}.tar.gz
    tar -xzf binutils-${gprofng_ver}.tar.gz
    rm -r binutils-${gprofng_ver}.tar.gz
    echo "Installing gprofng"
    cd binutils-${gprofng_ver}
    ./configure
    make
    make install
    cd ..
