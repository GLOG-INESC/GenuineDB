...
int main(int argc, char* argv[]) {
...
#ifndef SYSTEM_TIGA
  LOG(ERROR) << "Wrong configuration setup! System must be set up as Tiga";
  return -1;
#endif
  ...
  auto [storage, metadata_initializer] = slog::MakeSpecStorage(config, FLAGS_data_dir);
  ...
  modules.emplace_back(MakeRunnerFor<tiga::Coordinator>(broker->context(), broker->config(), metadata_initializer, metrics_manager),
                       slog::ModuleId::FORWARDER);
  modules.emplace_back(MakeRunnerFor<tiga::Acceptor>(broker->context(), broker->config(), metrics_manager),
                       slog::ModuleId::SEQUENCER);
  modules.emplace_back(MakeRunnerFor<tiga::Scheduler>(broker, storage, metrics_manager),
