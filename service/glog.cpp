...
int main(int argc, char* argv[]) {
  ...
  LOG(INFO) << "GLOG version: " << SLOG_VERSION;
...
#ifndef SYSTEM_GLOG
  LOG(ERROR) << "Wrong configuration setup! System must be set up as GLOG";
  return -1;
#endif

#ifndef LOCK_MANAGER_RMA
  LOG(ERROR) << "GLOG must be used in RMA Lock system";
  return -1;
#endif
  ...
  CHECK(!config->bypass_mh_orderer()) << "Wrong configuration. Bypass MH order must be disabled";
  ...
  if (std::chrono::high_resolution_clock::is_steady) {
    LOG(INFO) << "Using Steady Clocks";
  } else {
    LOG(INFO) << "Using System Clocks";
  }
  ...
  if (!config->shrink_stubs() || config->local_partition() == config->leader_partition_for_multi_home_ordering()) {
    modules.emplace_back(MakeRunnerFor<glog::GlogStub>(config->local_region(), config->local_replica(),
                                                       config->local_partition(), 0, broker, metrics_manager),
  ...
  if (config->local_partition() == config->leader_partition_for_multi_home_ordering()){
    modules.emplace_back(MakeRunnerFor<glog::GLOG>(0, broker->context(), broker->config(), metrics_manager),
                        slog::ModuleId::GLOG);
  }
