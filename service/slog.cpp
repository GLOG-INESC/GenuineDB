...
#if defined(PARTIAL_REP) && defined(FULL_EXEC)
  LOG(ERROR) << "Partial replication cannot be run with full execution";
  return -1;
#endif
...
#ifndef LOCK_MANAGER_DDR
  modules.emplace_back(MakeRunnerFor<slog::MultiHomeOrderer>(broker->context(), broker->config(), metrics_manager),
