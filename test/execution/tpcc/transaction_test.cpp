...
class TransactionTest : public ::testing::Test {
 protected:
  void SetUp() {
    ...
    auto metadata_initializer = std::make_shared<TPCCMetadataInitializer>(1, 1, 0);
    kv_storage_adapter = std::make_shared<KVStorageAdapter>(storage, metadata_initializer);
    LoadTables(kv_storage_adapter, W, 1, 1, 0, 0);
  }
...
TEST_F(TransactionTest, InspectStorage) {
  ...
  }
  {
    ...
    for (int i_id = 1; i_id <= 5; i_id++) {
      rows.push_back(item.Select({MakeInt32Scalar(1), MakeInt32Scalar(i_id)}));
    }
...
class PartialRepTransactionTest : public ::testing::Test {
 protected:
  void SetUp() {
    int num_regions = 2;
    ...
    asd.mutable_tpcc_partitioning()->set_warehouses(W);
    asd.set_execution_type(slog::internal::TPC_C);
    asd.set_glog_port(7);
    asd.set_order_port(7);
    auto configs = MakeTestConfigurations("asd", num_regions, 1, num_partitions, asd);

    for (int r = 0; r < num_regions; r++){
      for (int p = 0; p < num_partitions; p++){

        auto storage = std::make_shared<MemOnlyStorage>();
        auto metadata_initializer = std::make_shared<TPCCMetadataInitializer>(num_regions, num_partitions, 0);
        auto kv_storage_adapter = std::make_shared<KVStorageAdapter>(storage, metadata_initializer);
        LoadTables(kv_storage_adapter, W, num_regions, num_partitions, p, r);

        auto config = configs[p+r*num_partitions];

        auto sharder = Sharder::MakeSharder(config);

        regions.push_back({storage, metadata_initializer, kv_storage_adapter, config, sharder});
      }
    }

  }

  void FlushAndRefreshTxn(Transaction* t, std::shared_ptr<Storage> storage) {
    ...
    for (const auto& key : t->deleted_keys()) {
      ASSERT_TRUE(storage->Delete(key));
    }
    for (auto& kv : *(t->mutable_keys())) {
      const auto& key = kv.key();
      Record record;
      if (storage->Read(key, record)) {
        auto value = kv.mutable_value_entry();
    ...
    t->clear_deleted_keys();
  }

  Transaction txn;
  const int W = 4;
  std::vector<RegionStructs> regions;

};

TEST_F(PartialRepTransactionTest, NewOrderSingleRegion) {
  int w_id = 1;
  int d_id = 2;
  int c_id = 5;
  int o_id = 5000;
  ...
  {
    auto key_gen_adapter = std::make_shared<TxnKeyGenStorageAdapter>(txn);
    NewOrderTxn new_order_txn(key_gen_adapter, w_id, d_id, c_id, o_id, datetime, w_i_id, ol, true);
    new_order_txn.Read();
    new_order_txn.Write();
    key_gen_adapter->Finialize();
  }
  FlushAndRefreshTxn(&txn, regions[0].storage);
  {
    auto txn_adapter = std::make_shared<TxnStorageAdapter>(txn);
    NewOrderTxn new_order_txn(txn_adapter, w_id, d_id, c_id, o_id, datetime, w_i_id, ol);
    ASSERT_TRUE(new_order_txn.Execute());
...
TEST_F(PartialRepTransactionTest, NewOrderMultiRegion) {
  int w_id = 1;
  int d_id = 2;
  int c_id = 5;
  int o_id = 5000;
  ...
  std::array<NewOrderTxn::OrderLine, kLinePerOrder> ol;
  for (int i = 0; i < static_cast<int>(ol.size()); i++) {
    ol[i] = NewOrderTxn::OrderLine{.id = i + 1, .supply_w_id = 3, .item_id = (i + 1) * 10, .quantity = 4};
  }
  {
    auto key_gen_adapter = std::make_shared<TxnKeyGenStorageAdapter>(txn);
    NewOrderTxn new_order_txn(key_gen_adapter, w_id, d_id, c_id, o_id, datetime, w_i_id, ol, true);
  ...
  PopulateInvolvedMetadata(regions[0].metadata_initializer, txn);

  auto txn_w1 = GeneratePartitionedTxn(regions[0].sharder, &txn, 0, 0, false);

  PopulateInvolvedMetadata(regions[2].metadata_initializer, txn);
  auto txn_w2 = GeneratePartitionedTxn(regions[2].sharder, &txn, 1, 0, false);

  FlushAndRefreshTxn(txn_w1, regions[0].storage);
  {
    auto txn_adapter = std::make_shared<TxnStorageAdapter>(*txn_w1);
    NewOrderTxn new_order_txn(txn_adapter, w_id, d_id, c_id, o_id, datetime, w_i_id, ol);
    ASSERT_TRUE(new_order_txn.Execute());
  }

  FlushAndRefreshTxn(txn_w2, regions[2].storage);
  {
    auto txn_adapter = std::make_shared<TxnStorageAdapter>(*txn_w2);
    NewOrderTxn new_order_txn(txn_adapter, w_id, d_id, c_id, o_id, datetime, w_i_id, ol);
    ASSERT_TRUE(new_order_txn.Execute());
  }
}

TEST_F(PartialRepTransactionTest, PaymentSameRegion) {
  int w_id = 1;
  int d_id = 2;
  int c_w_id = 1;
  int c_d_id = 3;
  ...
  {
    auto key_gen_adapter = std::make_shared<TxnKeyGenStorageAdapter>(txn);
    PaymentTxn payment_txn(key_gen_adapter, w_id, d_id, c_w_id, c_d_id, c_id, amount, datetime, h_id, true);
    payment_txn.Read();
    payment_txn.Write();
    key_gen_adapter->Finialize();
  }
...
TEST_F(PartialRepTransactionTest, PaymentMultiRegion) {
  int w_id = 1;
  int d_id = 2;
  int c_w_id = 3;
  int c_d_id = 3;
