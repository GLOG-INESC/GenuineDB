...
class ForwarderTest : public ::testing::Test {
 protected:
  ...
  void SetUp() {
    ...
    configs = MakeTestConfigurations("forwarder", 2 /* num_regions */, 1 /* num_replicas */, 2 /* num_partitions */, custom_config);
...
TEST_F(ForwarderTest, ForwardToAnotherRegion) {
...

  {
    ...
    ASSERT_EQ(1, participating_partitions.size());
    std::set<MachineId> expected_participants;
    expected_participants.insert(MakeMachineId(0,0,0));
    ...
    ASSERT_EQ(0, active_partitions.size());
