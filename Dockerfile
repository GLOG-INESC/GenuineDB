FROM ubuntu:focal AS builder
    ...
    RUN apt-get update
    RUN apt-get -y install wget build-essential cmake git pkg-config iputils-ping
    ...
    RUN rm -rf build \
        && mkdir build \
        && cd build \
        && cmake .. -DBUILD_SLOG_TESTS=OFF -DCMAKE_BUILD_TYPE=release \
        && make -j$(nproc) \
        && cd ..
...
FROM ubuntu:focal AS runner
    ...
    COPY --from=builder /src/build/zip_order .
    COPY --from=builder /src/build/ziplog .
