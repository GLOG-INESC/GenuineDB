 ...
 public:
  ...
  static void LoadItem(const StorageAdapterPtr& storage_adapter, const std::vector<int>& rep_w) {
    ...
    LOG(INFO) << "Loading " << kMaxItems << " items for each of " << rep_w.size() << " representative warehouses [" << [&]() {
      ...
      for (size_t i = 0; i < rep_w.size(); ++i) {
        oss << rep_w[i];
        if (i + 1 < rep_w.size()) oss << ", ";
      }
      return oss.str();
    }() << "]";
...
#elif PARTIAL_REP
// Partition here must be in the form partition + region*num_partitions
void LoadTables(const StorageAdapterPtr& storage_adapter, int W, int num_regions, int num_partitions, int partition, int region,
                int num_threads) {
  LOG(INFO) << "Generating ~" << W / (num_partitions*num_regions) << " warehouses using " << num_threads << " threads. ";


  std::vector<int> rep_w;
  std::vector<std::thread> threads;
#ifdef PARTIAL_REP
  // We assume that the Item table is GLobally replicated.
  // We emulate this by loading items of all warehouses in all regions
  /*
  rep_w.reserve(W);
  for (int i = 0; i < W; i++) {
    rep_w.push_back(i + 1);
  ...
  */
  std::atomic<int> num_done = 0;
  auto LoadItemsFn = [&](int from_w, int to_w, int step) {

    std::vector<int> rep_x;

    for (int i = from_w; i < to_w; i += step) {
      rep_x.push_back(i);
    }


    if (!rep_x.empty()) {
      PartitionedTPCCDataLoader::LoadItem(storage_adapter, rep_x);
    }
    num_done++;
  };

  int item_alloc_range = W / num_threads + 1;

  for (int i = 0; i < num_threads; i++) {

    int range_start = i * item_alloc_range;
    int partition_of_range_start = range_start % (num_partitions);
    int distance_to_next_in_partition_key = (partition - partition_of_range_start + num_partitions) % (num_partitions);
    int from_key = range_start + distance_to_next_in_partition_key;
    int to_key = std::min((i + 1) * item_alloc_range, W);

    threads.emplace_back(LoadItemsFn, from_key + 1, to_key + 1, num_partitions);
  }

  while (num_done < threads.size()) {
    std::this_thread::sleep_for(std::chrono::seconds(5));
  }
...
#else
  int rep_w_counter = partition;
  for (int i = 0; i < (W / (num_partitions*num_regions)); i++) {
    rep_w.push_back(rep_w_counter + 1);
    rep_w_counter += num_partitions*num_regions;
  }
  PartitionedTPCCDataLoader::LoadItem(storage_adapter, rep_w);

#endif

  int global_partition = partition + region*num_partitions;

  auto LoadFn = [&](int from_w, int to_w, int seed) {
    PartitionedTPCCDataLoader loader(storage_adapter, from_w, to_w, num_regions*num_partitions, seed);
    loader.Load();
    num_done++;
  };
  int range = W / num_threads + 1;
  for (int i = 0; i < num_threads; i++) {
    int range_start = i * range;
    int partition_of_range_start = range_start % (num_partitions*num_regions);
    int distance_to_next_in_partition_key = (global_partition - partition_of_range_start + num_partitions*num_regions) % (num_partitions*num_regions);
    int from_key = range_start + distance_to_next_in_partition_key;
    int to_key = std::min((i + 1) * range, W);
    threads.emplace_back(LoadFn, from_key + 1, to_key + 1, i);
  }
