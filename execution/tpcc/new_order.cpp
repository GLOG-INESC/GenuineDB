...
NewOrderTxn::NewOrderTxn(const StorageAdapterPtr& storage_adapter, int w_id, int d_id, int c_id, int o_id,
                         int64_t datetime, int i_w_id, const std::array<OrderLine, kLinePerOrder>& ol, bool reconnaissance)
  ...
  // Reconnaissance flag to ignore partial replication
  if (reconnaissance){
    SetReconnaissance();
  }
}

// In Partial replication, it is okay for the warehouses to return "key does not exist".
// We assume that transactions already arrive correctly partitioned for execution
#ifdef PARTIAL_REP
bool NewOrderTxn::Read() {
  ...
  for (auto& l : a_ol_) {
    ...
    // We assume the one-shot transaction model of Janus/Tiga where dist_info is not propagated between partitions
    l.dist_info = MakeFixedTextScalar<24>(std::string(24, 'x'));
  }
...
void NewOrderTxn::Compute() {
  ...
  // Only local warehouse should compute this
  if (local_dist) {
  ...
  for (auto& l : a_ol_) {
    ...
    // Only (possibly remote) warehouse should compute this
    if (remote_stock) {
...
bool NewOrderTxn::Write() {
  ...
  for (const auto& l : a_ol_) {
    ...
    if (remote_stock || reconnaissance()) {
