...
class TPCCTransaction {
 public:
  ...
  bool reconnaissance() const { return  reconnaissance_; }
 ...
 protected:
  ...
  void SetReconnaissance() {
    reconnaissance_ = true;
  }

 private:
  std::string error_;
  bool reconnaissance_ = false;
...
class NewOrderTxn : public TPCCTransaction {
 public:
  ...
  NewOrderTxn(const StorageAdapterPtr& storage_adapter, int w_id, int d_id, int c_id, int o_id, int64_t datetime,
              int i_w_id, const std::array<OrderLine, kLinePerOrder>& ol, bool reconnaissance = false);
  bool Read() final;
  void Compute() final;
  bool Write() final;
 ...
 private:
  ...
  bool local_dist = false;
  bool remote_stock = false;
...
class PaymentTxn : public TPCCTransaction {
 public:
  PaymentTxn(const StorageAdapterPtr& storage_adapter, int w_id, int d_id, int c_w_id, int c_d_id, int c_id,
             int64_t amount, int64_t datetime, int h_id, bool reconnaissance = false);
  bool Read() final;
  void Compute() final;
  bool Write() final;
...
class OrderStatusTxn : public TPCCTransaction {
 public:
  OrderStatusTxn(const StorageAdapterPtr& storage_adapter, int w_id, int d_id, int c_id, int o_id, bool reconnaissance = false);
  bool Read() final;
  void Compute() final {}
...
class DeliverTxn : public TPCCTransaction {
 public:
  DeliverTxn(const StorageAdapterPtr& storage_adapter, int w_id, int d_id, int no_o_id, int c_id, int o_carrier,
             int64_t datetime, bool reconnaissance = false);
  bool Read() final;
  void Compute() final;
  bool Write() final;
...
class StockLevelTxn : public TPCCTransaction {
 public:
  ...
  StockLevelTxn(const StorageAdapterPtr& storage_adapter, int w_id, int d_id, int o_id,
                const std::array<int, kTotalItems>& i_ids, bool reconnaissance = false);
  bool Read() final;
  void Compute() final {}
  bool Write() final { return true; }
