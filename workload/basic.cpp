...
BasicWorkload::BasicWorkload(const ConfigurationPtr& config, RegionId region, ReplicaId replica, const string& data_dir,
  ...
  auto hot_keys_per_list = std::max(1U, params_.GetUInt32(HOT));
  CHECK(hot_keys_per_list >= params_.GetUInt32(HOT_RECORDS))
      << "Not enough HOT records per partition to meet the required treshold (" << hot_keys_per_list << " vs "
      << params_.GetUInt32(HOT_RECORDS) << ")";
  ...
  is_one_shot_workload_ = proto_config.one_shot_txns();
