...
//Percentage of read/write transactions
constexpr char WRITE_PCT[] = "rw";
...
SkewedWorkload::SkewedWorkload(const ConfigurationPtr& config, RegionId region, ReplicaId replica, const string& data_dir,
...
std::pair<Transaction*, TransactionProfile> SkewedWorkload::NextTransaction() {
  ...
  std::shuffle(selected_homes.begin(), selected_homes.end(), rg_);

  CHECK_LE(writes, records) << "Number of writes cannot exceed number of records in a transaction!";

  // Decide if the txn is read-only or read-write
  auto write_pct = params_.GetDouble(WRITE_PCT);

  bernoulli_distribution is_rw_txn(write_pct / 100);
  pro.is_rw = is_rw_txn(rg_);

  if (!pro.is_rw){
    writes = 0;
  }

  // Keep a counter per partition and home selected to always select the same keys
  std::vector<std::vector<int>> key_dist(num_regions, std::vector<int>(num_partitions, 0));
  ...
  for (size_t i = 0; i < records; i++) {
    ...
    Key key = std::to_string(num_partitions * (key_dist[home][partition]++ * num_regions + home) + partition);

    auto ins = pro.records.try_emplace(key, TransactionProfile::Record());

    CHECK(ins.second) << "Tried to insert already existing key";
  ...
  txn->mutable_internal()->set_is_one_shot_txn(is_one_shot_workload_);
