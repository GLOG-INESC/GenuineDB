...
std::pair<Transaction*, TransactionProfile> TPCCWorkload::NextTransaction() {
  ...
  switch (select_tpcc_txn(rg_)) {
    ...
    case 1:
      ...
      break;
    case 2:
      OrderStatus(*txn, pro, w);
      break;
    case 3:
      Deliver(*txn, pro, w);
      break;
    case 4:
      StockLevel(*txn, pro,w);
      break;
    default:
...
void TPCCWorkload::NewOrder(Transaction& txn, TransactionProfile& pro, int w_id, int partition) {
  ...
  std::bernoulli_distribution is_remote(0.1);
  ...
  if (is_remote(rg_) && !remote_warehouses.empty()){
    ...
    while (supply_w_id == w_id){
      supply_w_id = remote_warehouses[i++];
    }
  ...
  tpcc::NewOrderTxn new_order_txn(txn_adapter, w_id, d_id, c_id, o_id, datetime, i_w_id, ol, true /*reconnaissance*/);
  new_order_txn.Read();
  new_order_txn.Write();
...
void TPCCWorkload::Payment(Transaction& txn, TransactionProfile& pro, int w_id, int partition) {
  ...
  tpcc::PaymentTxn payment_txn(txn_adapter, w_id, d_id, c_w_id, c_d_id, c_id, amount, datetime, h_id, true /*reconnaissance*/);
  payment_txn.Read();
  payment_txn.Write();
  txn_adapter->Finialize();
...
void TPCCWorkload::OrderStatus(Transaction& txn, TransactionProfile& pro, int w_id) {
  auto txn_adapter = std::make_shared<tpcc::TxnKeyGenStorageAdapter>(txn);
...
void TPCCWorkload::Deliver(Transaction& txn, TransactionProfile& pro, int w_id) {
  auto txn_adapter = std::make_shared<tpcc::TxnKeyGenStorageAdapter>(txn);
  int c_id = NURand(rg_, 1023, 1, tpcc::kCustPerDist);
  auto d_id = std::uniform_int_distribution<>(1, tpcc::kDistPerWare)(rg_);
  ...
  tpcc::DeliverTxn deliver(txn_adapter, w_id, d_id, no_o_id, c_id, carrier, datetime, true /*reconnaissance*/);
  deliver.Read();
  deliver.Write();
  txn_adapter->Finialize();
...
void TPCCWorkload::StockLevel(Transaction& txn, TransactionProfile& pro, int w_id) {
  auto txn_adapter = std::make_shared<tpcc::TxnKeyGenStorageAdapter>(txn);
  ...
  tpcc::StockLevelTxn stock_level(txn_adapter, w_id, d_id, o_id, i_ids, true /*reconnaissance*/);
  stock_level.Read();
  txn_adapter->Finialize();
