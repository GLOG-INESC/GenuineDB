...
UniqueWorkload::UniqueWorkload(const ConfigurationPtr& config, RegionId region, ReplicaId replica, const string& data_dir,
                             const string& params_str, const uint64_t num_generators, const uint32_t workload_id, const uint64_t num_clients, const RawParamMap& extra_default_params)
    : BasicWorkload(config, region, replica, data_dir, params_str, 0, extra_default_params),
      num_generators_(num_generators){
  name_ = "unique";
  workload_id_ = region*num_generators + workload_id;

  CHECK(config->proto_config().has_simple_partitioning()) << "Unique only works with simple partitioning";
  auto total_records = config->proto_config().simple_partitioning().num_records();

  auto x = total_records % (config->num_partitions()*config->num_regions()*(config->num_regions()*num_generators)*num_clients*params_.GetUInt32(RECORDS));

  CHECK(x == 0) << " Number of records must match with number of regions, partitions, generators, clients and records";
  counters_ = vector<vector<uint64_t>>(config->num_partitions(), vector<uint64_t>(config->num_regions(), 0));

}

std::pair<Transaction*, TransactionProfile> UniqueWorkload::NextTransaction() {
  ...
  for (size_t i = 0; i < records; i++) {
    ...
    for (;;) {
      ...
      Key key = partition_to_key_lists_[partition][home].GetSpecificKey(workload_id_+ (counters_[partition][home]++)*(num_generators_*config_->num_regions()));

      auto ins = pro.records.try_emplace(key, TransactionProfile::Record());
      CHECK(ins.second) << "this method should never repeat the same key twice";
...
std::pair<Transaction*, TransactionProfile> UniqueWorkload::SpecificTransaction(std::vector<RegionId> regions, std::vector<PartitionId> partitions) {
  TransactionProfile pro;
  ...
  for (size_t i = 0; i < records; i++) {
    // Round-robin the partitions
    auto partition = partitions[i % partitions.size()];
    // Evenly divide the records to the selected homes
    auto home = regions[i / ((records + 1) / regions.size())];
