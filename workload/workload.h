...
class KeyList {
 public:
  ...
  KeyList(const ConfigurationPtr& config, int partition, int master, size_t num_hot_keys = 0)
    ...
    /*
    num_keys_ = (num_records / num_partitions_) / num_regions_;

    if ((num_records % (num_partitions_*num_regions_)) != 0) {

      throw std::runtime_error("Num Records must be equally split between regions and partitions");
    }
     */
    // num_keys_ = std::max(1LL, ((num_records - partition) / num_partitions_ - master) / num_regions_);
  }
  ...
  Key GetSpecificKey(uint64_t num_key) {
    uint64_t key = num_partitions_ * ((num_key % num_keys_) * num_regions_ + master_) + partition_;
    return std::to_string(key);
  }
...
class RegionKeyList {
 public:
  RegionKeyList(const ConfigurationPtr& config, int master, PartitionId partition, double zipf, const std::shared_ptr<ZipfDistribution>& zipf_dist)
      : is_simple_(true),  master_(master), local_partition_(partition) {
    auto simple_partitioning = config->proto_config().simple_partitioning();
    auto num_records = static_cast<long long>(simple_partitioning.num_records());
    num_regions_ = config->num_regions();
    num_partitions_ = config->num_partitions();

    num_keys_ = std::max(1LL, (num_records / num_regions_ / num_partitions_));



    if (zipf == 0.0){
      dist_ = std::make_shared<UniformDistribution>(num_keys_);
    } else {
      if (!zipf_dist->IsInitialized()){
        std::cout << "INITIALIZING" << std::endl;
        zipf_dist->Initialize(num_keys_, zipf);
      }
      dist_ = zipf_dist;
    }
  }
  ...
  Key GetKey(std::mt19937& rg) {
    // Local Region key
    auto r_key = dist_->GetKey(rg);

    // Translate to global key according to the partitioninig
    uint64_t g_key = local_partition_ + master_*num_partitions_ + r_key * num_regions_*num_partitions_;

    // Translate to global key according to the partitioning
    //uint64_t g_key = r_key + (master_ * num_partitions_) + (r_key / num_partitions_) * ((num_regions_-1) * num_partitions_);

    return std::to_string(g_key);
  }

  Key GetSpecificKey(uint64_t num_key) {
    uint64_t key = num_key + (master_ * num_partitions_) + (num_key / num_partitions_) * ((num_regions_-1) * num_partitions_);
    return std::to_string(key);
  }
 ...
 private:
  ...
  std::shared_ptr<AccessDistribution> dist_;
};



// Key list without considering partitioning
/*
class GlobalKeyList {
 public:
  GlobalKeyList(const ConfigurationPtr& config, const double zipf) : is_simple_(true) {
    auto simple_partitioning = config->proto_config().simple_partitioning();

    num_keys_ = static_cast<long long>(simple_partitioning.num_records());
  }

  Key GetRandomKey(std::mt19937& rg) {
    std::uniform_int_distribution<uint64_t> dis(0, num_keys_ - 1);

    return std::to_string(dis(rg));
  }
