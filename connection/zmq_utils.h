...
template <typename T>
inline bool RecvDeserializedProtoPoolRecv(zmq::socket_t& socket, T& out, bool dont_wait = false) {
  if (zmq::message_t empty; !socket.recv(empty, zmq::recv_flags::dontwait) || !empty.more()) {
    return false;
  }
  return RecvDeserializedProto(socket, out, dont_wait);
}
